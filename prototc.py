...
def cpp_store_struct(telemetries: list[Telemetry]) -> str:
    ...
    for trailer_telemetry in filter(lambda t: t.is_trailer_channel, telemetries):
        if trailer_telemetry.indexed:
            ...
            template_args: str = (
                f"{trailer_telemetry.scs_type}, {trailer_telemetry.max_count}"
            )
        else:
...
def registrations_for(
    ...
    for telemetry in telemetries:
        ...
        callback: str = (
            f"{STORE_FUNCTION_NAME}<{telemetry.scs_type}, {telemetry.max_count}>"
            if telemetry.indexed
            else f"store<{telemetry.scs_type}>"
        )
...
def offsets_of(
    ...
    if telemetry.is_trailer_channel or trailer_index != -1:
        ...
        return tuple(
            origin + offset
            for offset in trailer_data_offset_of(telemetries, telemetry, trailer_index)
        )
